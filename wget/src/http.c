...
{
	printf("You are currently waiting for the head reading process\n");
...
void  symbol_server_response(int __server_fd)
{

	printf("You are in the symbol server response function\n");

	char buf[STREAM_BUFFER_SIZE_WGET];

	int flags=0;
	int received=0;
	received=recv(__server_fd, buf, STREAM_BUFFER_SIZE_WGET, flags);

	printf("You have received %d bytes of message from the client\n", received);

	printf("The contents you received from the client is %s\n", buf);

	char symbuf[STREAM_BUFFER_SIZE_WGET];
	
	klee_make_symbolic(symbuf, STREAM_BUFFER_SIZE_WGET, "SymbolServer");	

	write(__server_fd, symbuf, STREAM_BUFFER_SIZE_WGET);
}
...
failed_tunnel:
	...
	printf("The position is here for http request sent***************************************\n");
	...
	symbol_server_response(__server_fd);
