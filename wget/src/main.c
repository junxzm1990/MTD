...
static void sym_server()
{
	...
	serv_addr.sin_addr.s_addr = htonl(DEFAULT_NETWORK_ADDR);
	serv_addr.sin_port = htons(80); 

	bind(listenfd, (struct sockaddr*)&serv_addr, sizeof(serv_addr)); 
	listen(listenfd, 10); 
	__server_fd=listenfd;
}
...
# define FOPEN_OPT_ARGS
	...
	for (t = url; *t; t++)
	{
		...
		else
		{
			...
			{
				printf("You are at the else section for retrieval_url\n");
